...
function saveComplainDataHelp($user_id,$issue_in,$type_of_issue,$description,$ltn,$db_helper){
	$issue_details=[];
	...
	if(empty($ltn)){
		$response= ["status_code"=>400,"message"=>"Login type number can't be empty .."];
		return $response;
	}
	...
	if(isset($issue_details[$issue_in])){
		...
		if(array_search($type_of_issue,$issue_details[$issue_in])!==false){
	        ...
	        $ci->db->where([
	            ...
	            'login_type_number' => $ltn
	        ]);

	        $complain = $ci->db->get('complaint_form')->row();
	        //check for same complaint not registered again
	        if($complain){
	        	$response= ["status_code"=>200,"message"=>"Complaint already registered.."];
				return $response;
	        }
...
function generate_unique_key() {
        // Get the current datetime in a specific format (e.g., YmdHis for YearMonthDayHourMinuteSecond)
        $datetime = date('YmdHis');
        
        // Generate a random string of 4 unique characters
        ...
        // Concatenate datetime and random string to form the unique key
        $uniqueKey = $datetime . $randomString;
        
        return $uniqueKey;
    }

function prakash($arr){
	echo "<pre>";
	print_r($arr);
	die();
}
