...
<script type="text/javascript">
    jQuery(document).ready(function(){
        ...
        $("#submitBerth").click(function(event){

            event.preventDefault();
            var berth=$("#berth").val();
            if(berth==""){
                alert("Please enter berth no.");
                return;
            }
            // console.log(CoachNo+CoachCat);
            $.ajax({

                url:'<?php echo base_url("Master/saveBerth"); ?>',
                method:'POST',
                data:{
                    berth:berth,
                },
                success:function(response){
                    alert(response);
                    window.location.href='<?php echo base_url("Master/addBerth"); ?>';
